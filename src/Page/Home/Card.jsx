...
const Card = ({ item }) => {
    const { fruit_name, img, price} = item
    return (
        <div>
            <div className="md:p-6 rounded-md shadow-md border-2 border-gray-300">
                <img src={img} className="object-cover md:object-center w-full rounded-md h-full md:h-72 " />
                <div className="mt-6 mb-2 text-center space-y-2">
                    <p className="text-xl font-bold border-b-2 pb-1 border-black">{price}</p>
                    <p className="text-base ">{fruit_name}</p>
                    {/* rating */}
